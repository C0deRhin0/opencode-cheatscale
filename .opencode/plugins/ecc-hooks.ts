    ...
    "session.idle": async () => {
      ...
      if (roadmapPath) {
        ...
        try {
          ...
          // Validate logic rules from the "Fresh Start Mandate"
          const hasPhase0 = content.includes("### Phase 0");
          
          // Dynamically check ALL generated phases (whether 2 or 20)
          const phases = content.split("### Phase").slice(1);
          let invalidPhase = false;
          
          for (const phase of phases) {
            // Count how many "**Day" declarations are in this specific phase
            const dayCount = (phase.match(/\*\*Day/g) || []).length;
            if (dayCount < 2) {
              invalidPhase = true;
              break;
            }
          ...
          if (!hasPhase0) {
            log("error", "[QUALITY GATE FAILED] roadmap.md must start with Phase 0. Tell the AI to fix this.");
          } else if (invalidPhase) {
            log("error", "[QUALITY GATE FAILED] Found a Phase with fewer than 2 Days. Every Phase MUST have multiple days. Tell the AI to fix this.");
          } else {
            log("info", `[QUALITY GATE PASSED] Validated ${phases.length} Phases dynamically with correct day distributions.`);
          }
        } catch {
          log("error", "[QUALITY GATE] Failed to parse roadmap.md");
        }
      }
