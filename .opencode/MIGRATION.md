...
.opencode/
 ...
 commands/                  # 37+ registered commands (markdown)
    plan.md
    tdd.md
    ... (and more)
 instructions/
...
After migration, ALL 37+ commands are available. Core commands:
...
| `/task-orbit` | Phase-aware atomic checkpointing |
| `/push` | Drip-feeder DevOps push |
| `/commit` | Buffer work into local queue |
| `/sitrep` | Fast situational report of workspace state |
| `/debate` | Multi-agent synthesis |
| `/research` | Adversarial research loop |
| `/docs` | Live documentation lookup via Context7 |
| `/multi-plan` | Multi-perspective planning (backend + frontend) |
| `/multi-execute` | Execute a multi-plan with full audit |
| `/multi-backend` | Backend-focused 6-phase workflow |
| `/multi-frontend` | Frontend-focused 6-phase workflow |
| `/multi-workflow` | Full-stack development workflow |
| `/loop-start` | Start a managed autonomous loop |
| `/loop-status` | Inspect active loop state |
| `/harness-audit` | Deterministic harness scorecard |
...
|---------|-------------|----------|--------|
| Agents | PASS: 14 agents | PASS: 20 agents | **OpenCode has MORE** |
| Commands | PASS: 28 commands | PASS: 37+ commands | **OpenCode has MORE** |
| Skills | PASS: 16 skills | PASS: 29 skills | **OpenCode has MORE** |
